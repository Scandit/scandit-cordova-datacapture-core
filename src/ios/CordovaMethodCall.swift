...
/// is expected to be a dictionary containing the actual method arguments.
public class CordovaMethodCall: FrameworksMethodCall {

    private let command: CDVInvokedUrlCommand

    public init(command: CDVInvokedUrlCommand) {
        self.command = command
    }

    private var argumentsDictionary: [String: Any] {
        guard let dict = command.defaultArgumentAsDictionary else {
            return [:]
        }
        // Filter out nil values and convert [String: Any?] to [String: Any]
        return dict.compactMapValues { $0 }
    }

    public var method: String {
        argumentsDictionary["methodName"] as? String ?? "unknown"
    }

    public func argument<T>(key: String) -> T? {
        guard let value = argumentsDictionary[key] else {
            return nil
        }
    ...
    public func hasArgument(key: String) -> Bool {
        argumentsDictionary.keys.contains(key)
    }
